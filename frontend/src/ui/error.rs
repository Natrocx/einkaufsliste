...
impl From<ApiError> for GuiMessage {
  fn from(error: ApiError) -> Self {
    Self::ApiError(Arc::new(error))
  }
}

// The following code has been taken from https://github.com/iced-rs/iced/blob/master/examples/toast/src/main.rs
// Copyright: The Iced Authors, Jonas Lauschke
// License: MIT
...
use iced::{
  advanced, mouse, theme, window, Alignment, Element, Length, Point, Rectangle, Renderer, Size, Theme, Vector,
};
...
pub enum Status {
  #[default]
  /// An "Info" level status
  Primary,
  Secondary,
  Success,
  /// An unexpected (programming) error occurred
  Danger,
}
...
/// A wrapper for displaying temporary messages to the user. Primarily used to show error messages.
...
impl<'a, Message> Widget<Message, Renderer> for Manager<'a, Message> {
  ...
  fn operate(&self, state: &mut Tree, layout: Layout<'_>, renderer: &Renderer, operation: &mut dyn Operation<Message>) {
    ...
    if let Event::Window(window::Event::RedrawRequested(now)) = &event {
      ...
      self.instants.iter_mut().enumerate().for_each(|(index, maybe_instant)| {
        if let Some(instant) = maybe_instant.as_mut() {
          let remaining = Duration::from_secs(self.timeout_secs).saturating_sub(instant.elapsed());
